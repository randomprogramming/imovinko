import React from "react";

export type IconName =
    | "search"
    | "account"
    | "google"
    | "loading"
    | "logout"
    | "house-plus"
    | "house-colorful"
    | "apartment-colorful"
    | "land-colorful"
    | "marker"
    | "logo-text"
    | "logo"
    | "image-plus"
    | "heart"
    | "close"
    | "left"
    | "right"
    | "area"
    | "bed"
    | "bath"
    | "car"
    | "show-more"
    | "list"
    | "cards"
    | "office"
    | "account-settings"
    | "property"
    | "account-plus"
    | "filter"
    | "checkmark"
    | "notification"
    | "success"
    | "solo-arrow"
    | "missing-image"
    | "down-chevron"
    | "traffic"
    | "walking"
    | "cycling"
    | "location"
    | "file-missing"
    | "warning"
    | "error"
    | "image-edit"
    | "edit"
    | "sold"
    | "trash"
    | "send"
    | "three-dots"
    | "help"
    | "star"
    | "share"
    | "information"
    | "facebook"
    | "twitter"
    | "file-upload"
    | "heating";

interface IconProps {
    name: IconName;
    height?: string | number;
    width?: string | number;
    className?: string;
}

const DEFAULT_W = "24";
const DEFAULT_H = "24";
export default function Icon({ name, height, width, className }: IconProps) {
    switch (name) {
        case "search":
            return (
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                >
                    <g
                        fill="none"
                        stroke="currentColor"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth="2"
                    >
                        <circle cx="11" cy="11" r="8" />
                        <path d="m21 21l-4.35-4.35" />
                    </g>
                </svg>
            );
        case "account":
            return (
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                >
                    <path
                        className={className}
                        fill="currentColor"
                        d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2zm0 4c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07 6 12 6zm0 14c-2.03 0-4.43-.82-6.14-2.88a9.947 9.947 0 0 1 12.28 0C16.43 19.18 14.03 20 12 20z"
                    />
                </svg>
            );
        case "google":
            return (
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 48 48"
                >
                    <path
                        fill="#FFC107"
                        d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4C12.955 4 4 12.955 4 24s8.955 20 20 20s20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z"
                    />
                    <path
                        fill="#FF3D00"
                        d="m6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4C16.318 4 9.656 8.337 6.306 14.691z"
                    />
                    <path
                        fill="#4CAF50"
                        d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238A11.91 11.91 0 0 1 24 36c-5.202 0-9.619-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z"
                    />
                    <path
                        fill="#1976D2"
                        d="M43.611 20.083H42V20H24v8h11.303a12.04 12.04 0 0 1-4.087 5.571l.003-.002l6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z"
                    />
                </svg>
            );
        case "loading":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    aria-hidden="true"
                    className={`text-gray-200 animate-spin dark:text-gray-600 fill-white ${className}`}
                    viewBox="0 0 100 101"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                        fill="currentColor"
                    />
                    <path
                        d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                        fill="currentFill"
                    />
                </svg>
            );
        case "logout":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            d="M21 12L13 12"
                            stroke="#fafafa"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            className={className}
                        ></path>
                        <path
                            d="M18 15L20.913 12.087V12.087C20.961 12.039 20.961 11.961 20.913 11.913V11.913L18 9"
                            stroke="#fafafa"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            className={className}
                        ></path>
                        <path
                            d="M16 5V4.5V4.5C16 3.67157 15.3284 3 14.5 3H5C3.89543 3 3 3.89543 3 5V19C3 20.1046 3.89543 21 5 21H14.5C15.3284 21 16 20.3284 16 19.5V19.5V19"
                            stroke="#fafafa"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            className={className}
                        ></path>
                    </g>
                </svg>
            );
        case "house-plus":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    stroke=""
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <g>
                            <path
                                d="M12 16.0001V13.0001M12 13.0001V10.0001M12 13.0001H9M12 13.0001H15M4 16.8002V11.4522C4 10.9179 4 10.6506 4.06497 10.4019C4.12255 10.1816 4.2173 9.97307 4.34521 9.78464C4.48955 9.57201 4.69064 9.39569 5.09277 9.04383L9.89436 4.84244C10.6398 4.19014 11.0126 3.86397 11.4324 3.73982C11.8026 3.63035 12.1972 3.63035 12.5674 3.73982C12.9875 3.86406 13.3608 4.19054 14.1074 4.84383L18.9074 9.04383C19.3096 9.39569 19.5102 9.57201 19.6546 9.78464C19.7825 9.97307 19.8775 10.1816 19.9351 10.4019C20 10.6505 20 10.9184 20 11.4522V16.8037C20 17.9216 20 18.4811 19.7822 18.9086C19.5905 19.2849 19.2842 19.5906 18.9079 19.7823C18.4805 20.0001 17.9215 20.0001 16.8036 20.0001H7.19691C6.07899 20.0001 5.5192 20.0001 5.0918 19.7823C4.71547 19.5906 4.40973 19.2849 4.21799 18.9086C4 18.4807 4 17.9203 4 16.8002Z"
                                stroke="#fafafa"
                                strokeWidth="2"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                                className={className}
                            ></path>
                        </g>
                    </g>
                </svg>
            );
        case "apartment-colorful":
            return (
                <svg
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    fill="#000000"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinejoin="round"></g>
                    <g>
                        <g>
                            <polygon
                                fill="#FFD791"
                                points="200.417,102.079 106.355,8.016 12.292,102.079 12.292,460.87 200.417,460.87 "
                            ></polygon>
                            <rect
                                x="12.292"
                                y="460.875"
                                fill="#FFD791"
                                width="188.125"
                                height="43.108"
                            ></rect>
                        </g>
                        <g>
                            <path
                                fill="6DDAE1"
                                d="M106.355,67.874c-18.891,0-34.205,15.314-34.205,34.205v76.96h68.409v-76.96 C140.559,83.189,125.245,67.874,106.355,67.874z"
                            ></path>
                            <rect
                                x="46.497"
                                y="238.897"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="123.457"
                                y="238.897"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="46.497"
                                y="332.96"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="123.457"
                                y="332.96"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                        </g>
                        <polygon
                            fill="#FFD791"
                            points="499.708,503.983 311.582,503.983 311.582,102.079 405.645,8.016 499.708,102.079 "
                        ></polygon>
                        <g>
                            <rect
                                x="12.292"
                                y="460.875"
                                fill="#F8BE73"
                                width="188.125"
                                height="43.108"
                            ></rect>
                            <rect
                                x="311.582"
                                y="460.875"
                                fill="#F8BE73"
                                width="188.125"
                                height="43.108"
                            ></rect>
                        </g>
                        <g>
                            <path
                                fill="6DDAE1"
                                d="M405.645,67.874c-18.891,0-34.205,15.314-34.205,34.205v76.96h68.409v-76.96 C439.85,83.189,424.536,67.874,405.645,67.874z"
                            ></path>
                            <rect
                                x="345.787"
                                y="238.897"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="422.747"
                                y="238.897"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="345.787"
                                y="332.96"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="422.747"
                                y="332.96"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                        </g>
                        <rect
                            x="200.417"
                            y="102.079"
                            fill="#999999"
                            width="111.165"
                            height="401.904"
                        ></rect>
                        <g>
                            <rect
                                x="286.282"
                                y="102.079"
                                fill="#808080"
                                width="25.297"
                                height="401.904"
                            ></rect>
                            <rect
                                x="200.417"
                                y="102.079"
                                fill="#808080"
                                width="26.009"
                                height="401.904"
                            ></rect>
                        </g>
                        <g>
                            <rect
                                x="234.622"
                                y="144.835"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="234.622"
                                y="238.897"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="234.622"
                                y="332.96"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                            <rect
                                x="234.622"
                                y="427.023"
                                fill="6DDAE1"
                                width="42.756"
                                height="34.205"
                            ></rect>
                        </g>
                        <circle fill="#C4DF64" cx="448.401" cy="435.574" r="34.205"></circle>
                        <path
                            fill="#96CA63"
                            d="M465.503,452.676c-18.891,0-34.205-15.314-34.205-34.205c0-5.018,1.089-9.778,3.03-14.072 c-11.869,5.367-20.133,17.301-20.133,31.174c0,18.891,15.314,34.205,34.205,34.205c13.873,0,25.807-8.264,31.174-20.133 C475.281,451.586,470.52,452.676,465.503,452.676z"
                        ></path>
                        <circle fill="#C4DF64" cx="362.889" cy="435.574" r="34.205"></circle>
                        <path
                            fill="#96CA63"
                            d="M379.992,452.676c-18.891,0-34.205-15.314-34.205-34.205c0-5.018,1.089-9.778,3.03-14.072 c-11.869,5.367-20.133,17.301-20.133,31.174c0,18.891,15.314,34.205,34.205,34.205c13.873,0,25.807-8.264,31.174-20.133 C389.77,451.586,385.009,452.676,379.992,452.676z"
                        ></path>
                        <circle fill="#C4DF64" cx="149.111" cy="435.574" r="34.205"></circle>
                        <path
                            fill="#96CA63"
                            d="M166.213,452.676c-18.891,0-34.205-15.314-34.205-34.205c0-5.018,1.089-9.778,3.03-14.072 c-11.869,5.367-20.133,17.301-20.133,31.174c0,18.891,15.314,34.205,34.205,34.205c13.873,0,25.807-8.264,31.174-20.133 C175.991,451.586,171.23,452.676,166.213,452.676z"
                        ></path>
                        <circle fill="#C4DF64" cx="63.599" cy="435.574" r="34.205"></circle>
                        <path
                            fill="#96CA63"
                            d="M80.701,452.676c-18.891,0-34.205-15.314-34.205-34.205c0-5.018,1.089-9.778,3.03-14.072 c-11.869,5.367-20.133,17.301-20.133,31.174c0,18.891,15.314,34.205,34.205,34.205c13.873,0,25.807-8.264,31.174-20.133 C90.48,451.586,85.719,452.676,80.701,452.676z"
                        ></path>
                        <path d="M106.355,59.858c-23.281,0-42.221,18.94-42.221,42.221v76.96c0,4.427,3.588,8.017,8.017,8.017h68.409 c4.428,0,8.017-3.589,8.017-8.017v-76.96C148.576,78.798,129.635,59.858,106.355,59.858z M106.355,75.891 c11.645,0,21.534,7.642,24.928,18.171H81.427C84.821,83.533,94.71,75.891,106.355,75.891z M80.167,110.096h18.171v60.927H80.167 V110.096z M114.371,171.023v-60.927h18.171v60.927H114.371z"></path>
                        <path d="M38.48,238.897v34.205c0,4.427,3.588,8.017,8.017,8.017h42.756c4.428,0,8.017-3.589,8.017-8.017v-34.205 c0-4.427-3.588-8.017-8.017-8.017H46.497C42.068,230.881,38.48,234.47,38.48,238.897z M54.513,246.914h26.722v18.171H54.513V246.914 z"></path>
                        <path d="M174.23,238.897c0-4.427-3.588-8.017-8.017-8.017h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205 c0,4.427,3.588,8.017,8.017,8.017h42.756c4.428,0,8.017-3.589,8.017-8.017V238.897z M158.196,265.085h-26.722v-18.171h26.722 V265.085z"></path>
                        <path d="M38.48,367.165c0,4.427,3.588,8.017,8.017,8.017h42.756c4.428,0,8.017-3.589,8.017-8.017V332.96 c0-4.427-3.588-8.017-8.017-8.017H46.497c-4.428,0-8.017,3.589-8.017,8.017V367.165z M54.513,340.977h26.722v18.171H54.513V340.977z "></path>
                        <path d="M166.213,324.943h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205c0,4.427,3.588,8.017,8.017,8.017h42.756 c4.428,0,8.017-3.589,8.017-8.017V332.96C174.23,328.533,170.64,324.943,166.213,324.943z M158.196,359.148h-26.722v-18.171h26.722 V359.148z"></path>
                        <path d="M499.708,144.3c4.428,0,8.017-3.589,8.017-8.017v-34.205c0-2.126-0.844-4.165-2.347-5.668L411.314,2.348 c-3.132-3.131-8.207-3.131-11.338,0l-91.715,91.714H203.738L112.024,2.348c-3.132-3.131-8.207-3.131-11.338,0L6.624,96.411 c-1.504,1.503-2.348,3.541-2.348,5.668v188.125c0,4.427,3.588,8.017,8.017,8.017s8.017-3.589,8.017-8.017V105.399l86.046-86.046 l86.046,86.046v390.567h-35.273v-18.944c19.459-3.757,34.205-20.909,34.205-41.448c0-23.28-18.941-42.221-42.221-42.221 s-42.221,18.941-42.221,42.221c0,20.54,14.745,37.691,34.205,41.448v18.944H71.616v-18.944 c19.459-3.757,34.205-20.909,34.205-41.448c0-23.28-18.941-42.221-42.221-42.221s-42.221,18.94-42.221,42.221 c0,20.54,14.745,37.691,34.205,41.448v18.944H20.309V332.96c0-4.427-3.588-8.017-8.017-8.017s-8.017,3.589-8.017,8.017v171.023 c0,4.427,3.588,8.017,8.017,8.017h487.415c4.428,0,8.017-3.589,8.017-8.017V179.039c0-4.427-3.588-8.017-8.017-8.017 s-8.017,3.589-8.017,8.017v316.927h-35.273v-18.944c19.459-3.757,34.205-20.909,34.205-41.448c0-23.28-18.941-42.221-42.221-42.221 c-23.281,0-42.221,18.941-42.221,42.221c0,20.54,14.745,37.691,34.205,41.448v18.944h-69.478v-18.944 c19.459-3.757,34.205-20.909,34.205-41.448c0-23.28-18.941-42.221-42.221-42.221c-23.28,0-42.221,18.941-42.221,42.221 c0,20.54,14.745,37.691,34.205,41.448v18.944h-35.273V105.399l86.046-86.046l86.046,86.046v30.885 C491.691,140.711,495.279,144.3,499.708,144.3z M122.923,435.574c0-14.44,11.747-26.188,26.188-26.188 c14.441,0,26.188,11.747,26.188,26.188s-11.747,26.188-26.188,26.188S122.923,450.014,122.923,435.574z M37.411,435.574 c0-14.441,11.747-26.188,26.188-26.188s26.188,11.747,26.188,26.188s-11.747,26.188-26.188,26.188S37.411,450.014,37.411,435.574z M422.213,435.574c0-14.44,11.747-26.188,26.188-26.188c14.441,0,26.188,11.747,26.188,26.188s-11.747,26.188-26.188,26.188 C433.96,461.762,422.213,450.014,422.213,435.574z M336.701,435.574c0-14.44,11.747-26.188,26.188-26.188 s26.188,11.747,26.188,26.188s-11.747,26.188-26.188,26.188S336.701,450.014,336.701,435.574z M208.434,110.096h95.132v385.871 h-95.132V110.096z"></path>
                        <path d="M439.85,187.056c4.428,0,8.017-3.589,8.017-8.017v-76.96c0-23.281-18.941-42.221-42.221-42.221 c-23.28,0-42.221,18.94-42.221,42.221v76.96c0,4.427,3.588,8.017,8.017,8.017H439.85z M413.662,171.023v-60.927h18.171v60.927 H413.662z M405.645,75.891c11.645,0,21.534,7.642,24.928,18.171h-49.856C384.111,83.533,394,75.891,405.645,75.891z M379.457,110.096h18.171v60.927h-18.171V110.096z"></path>
                        <path d="M396.559,238.897c0-4.427-3.588-8.017-8.017-8.017h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205 c0,4.427,3.588,8.017,8.017,8.017h42.756c4.428,0,8.017-3.589,8.017-8.017V238.897z M380.526,265.085h-26.722v-18.171h26.722 V265.085z"></path>
                        <path d="M473.52,238.897c0-4.427-3.588-8.017-8.017-8.017h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205 c0,4.427,3.588,8.017,8.017,8.017h42.756c4.428,0,8.017-3.589,8.017-8.017V238.897z M457.486,265.085h-26.722v-18.171h26.722 V265.085z"></path>
                        <path d="M388.543,324.943h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205c0,4.427,3.588,8.017,8.017,8.017h42.756 c4.428,0,8.017-3.589,8.017-8.017V332.96C396.559,328.533,392.97,324.943,388.543,324.943z M380.526,359.148h-26.722v-18.171h26.722 V359.148z"></path>
                        <path d="M422.747,375.181h42.756c4.428,0,8.017-3.589,8.017-8.017V332.96c0-4.427-3.588-8.017-8.017-8.017h-42.756 c-4.428,0-8.017,3.589-8.017,8.017v34.205C414.731,371.592,418.319,375.181,422.747,375.181z M430.764,340.977h26.722v18.171 h-26.722V340.977z"></path>
                        <path d="M277.378,136.818h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205c0,4.427,3.588,8.017,8.017,8.017h42.756 c4.428,0,8.017-3.589,8.017-8.017v-34.205C285.394,140.407,281.805,136.818,277.378,136.818z M269.361,171.023h-26.722v-18.171 h26.722V171.023z"></path>
                        <path d="M277.378,230.881h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205c0,4.427,3.588,8.017,8.017,8.017h42.756 c4.428,0,8.017-3.589,8.017-8.017v-34.205C285.394,234.47,281.805,230.881,277.378,230.881z M269.361,265.085h-26.722v-18.171 h26.722V265.085z"></path>
                        <path d="M277.378,324.943h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205c0,4.427,3.588,8.017,8.017,8.017h42.756 c4.428,0,8.017-3.589,8.017-8.017V332.96C285.394,328.533,281.805,324.943,277.378,324.943z M269.361,359.148h-26.722v-18.171 h26.722V359.148z"></path>
                        <path d="M277.378,419.006h-42.756c-4.428,0-8.017,3.589-8.017,8.017v34.205c0,4.427,3.588,8.017,8.017,8.017h42.756 c4.428,0,8.017-3.589,8.017-8.017v-34.205C285.394,422.595,281.805,419.006,277.378,419.006z M269.361,453.211h-26.722v-18.171 h26.722V453.211z"></path>
                    </g>
                </svg>
            );
        case "house-colorful":
            return (
                <svg
                    version="1.0"
                    xmlns="http://www.w3.org/2000/svg"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 64 64"
                    enableBackground="new 0 0 64 64"
                    fill="#000000"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <g>
                            <path
                                fill="#F9EBB2"
                                d="M56,60c0,1.104-0.896,2-2,2H38V47c0-0.553-0.447-1-1-1H27c-0.553,0-1,0.447-1,1v15H10c-1.104,0-2-0.896-2-2 V31.411L32.009,7.403L56,31.394V60z"
                            ></path>
                            <polygon
                                fill="#F76D57"
                                points="14,6 18,6 18,12.601 14,16.593 "
                            ></polygon>
                            <rect x="28" y="48" fill="#F9EBB2" width="8" height="14"></rect>
                            <path
                                fill="#F76D57"
                                d="M61,33c-0.276,0-0.602-0.036-0.782-0.217L32.716,5.281c-0.195-0.195-0.451-0.293-0.707-0.293 s-0.512,0.098-0.707,0.293L3.791,32.793C3.61,32.974,3.276,33,3,33c-0.553,0-1-0.447-1-1c0-0.276,0.016-0.622,0.197-0.803 L31.035,2.41c0,0,0.373-0.41,0.974-0.41s0.982,0.398,0.982,0.398l28.806,28.805C61.978,31.384,62,31.724,62,32 C62,32.552,61.553,33,61,33z"
                            ></path>
                            <g>
                                <path
                                    fill="#394240"
                                    d="M63.211,29.789L34.438,1.015c0,0-0.937-1.015-2.43-1.015s-2.376,0.991-2.376,0.991L20,10.604V5 c0-0.553-0.447-1-1-1h-6c-0.553,0-1,0.447-1,1v13.589L0.783,29.783C0.24,30.326,0,31.172,0,32c0,1.656,1.343,3,3,3 c0.828,0,1.662-0.251,2.205-0.794L6,33.411V60c0,2.211,1.789,4,4,4h44c2.211,0,4-1.789,4-4V33.394l0.804,0.804 C59.347,34.739,60.172,35,61,35c1.657,0,3-1.343,3-3C64,31.171,63.754,30.332,63.211,29.789z M14,6h4v6.601l-4,3.992V6z M36,62h-8 V48h8V62z M56,60c0,1.104-0.896,2-2,2H38V47c0-0.553-0.447-1-1-1H27c-0.553,0-1,0.447-1,1v15H10c-1.104,0-2-0.896-2-2V31.411 L32.009,7.403L56,31.394V60z M61,33c-0.276,0-0.602-0.036-0.782-0.217L32.716,5.281c-0.195-0.195-0.451-0.293-0.707-0.293 s-0.512,0.098-0.707,0.293L3.791,32.793C3.61,32.974,3.276,33,3,33c-0.553,0-1-0.447-1-1c0-0.276,0.016-0.622,0.197-0.803 L31.035,2.41c0,0,0.373-0.41,0.974-0.41s0.982,0.398,0.982,0.398l28.806,28.805C61.978,31.384,62,31.724,62,32 C62,32.552,61.553,33,61,33z"
                                ></path>
                                <path
                                    fill="#394240"
                                    d="M23,32h-8c-0.553,0-1,0.447-1,1v8c0,0.553,0.447,1,1,1h8c0.553,0,1-0.447,1-1v-8 C24,32.447,23.553,32,23,32z M22,40h-6v-6h6V40z"
                                ></path>
                                <path
                                    fill="#394240"
                                    d="M41,42h8c0.553,0,1-0.447,1-1v-8c0-0.553-0.447-1-1-1h-8c-0.553,0-1,0.447-1,1v8 C40,41.553,40.447,42,41,42z M42,34h6v6h-6V34z"
                                ></path>
                            </g>
                            <rect x="28" y="48" fill="#506C7F" width="8" height="14"></rect>
                            <g>
                                <rect x="16" y="34" fill="#45AAB8" width="6" height="6"></rect>
                                <rect x="42" y="34" fill="#45AAB8" width="6" height="6"></rect>
                            </g>
                        </g>
                    </g>
                </svg>
            );
        case "land-colorful":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 340 340"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinejoin="round"></g>
                    <g>
                        <title></title>
                        <path
                            d="M16.431,211.357S8.482,169.612,38.657,123.1a38.609,38.609,0,0,1,8.648-9.46S76.626,100.493,95.6,118.9l31.155,31.439c12.14,11.973,32.5,17.236,48.432,11.151l18.694-7.753c14.627-5.588,26.767-6.007,39.035,3.725l7.2,3.362,33.484,26.564v32.287Z"
                            fill="#99a843"
                        ></path>
                        <path
                            d="M330.576,211.357s1.54-15.623-9.722-20.681a32.639,32.639,0,0,0,.668-6.392c.212-17.223-19.156-27.5-33.452-17.887-4.35,2.924-8.892,7.5-12.893,14.608,0,0-15.256-7.451-28.029,12.773,0,0-10.644-1.065-21.643,9.934,0,0-7.805,7.645-18.094,7.645v8.321H330.576Z"
                            fill="#737722"
                        ></path>
                        <path
                            d="M10.576,211.357s-5.961-24,8.435-41.117c9.679-11.51,28.429-8.7,33.541,5.439a26.739,26.739,0,0,1,1.513,8.164s18.068-7.1,24.149,11c0,0,14.136-10.289,25.542,4.258,0,0,9.278,12.257,24.889,12.257v6.9Z"
                            fill="#737722"
                        ></path>
                        <rect
                            height="110.136"
                            fill="#edf46c"
                            width="320"
                            x="10.576"
                            y="211.357"
                        ></rect>
                        <circle cx="253.668" cy="53.486" r="34.979" fill="#f74f73"></circle>
                        <path
                            d="M270.5,258.281,286.5,211.357h-8.452l-15.993,46.924H174.111L190.1,211.357h-8.451l-15.994,46.924H75.125l15.993-46.924H82.666L66.673,258.281h-56.1v8H40.748L21.93,321.493h8.452L49.2,266.281h71.13l-18.819,55.212h8.452l18.819-55.212h78.553l-18.819,55.212h8.452l18.819-55.212h72.4l-18.819,55.212h8.452l18.818-55.212h33.939v-8Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M27.437,249.984a3.5,3.5,0,0,1-3.5-3.5,34.843,34.843,0,0,0-1.673-10.268,3.5,3.5,0,0,1,6.6-2.347,41.522,41.522,0,0,1,2.078,12.615A3.5,3.5,0,0,1,27.437,249.984Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M38.5,249.984c-.071,0-.143,0-.215-.007a3.5,3.5,0,0,1-3.282-3.705c.034-.566.95-13.964,10.425-21.963a3.5,3.5,0,1,1,4.515,5.348C42.737,235.745,42,246.586,42,246.7A3.5,3.5,0,0,1,38.5,249.984Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M49.313,249.985a3.5,3.5,0,0,1-3.349-4.523A14.06,14.06,0,0,1,52.2,237.8a3.5,3.5,0,0,1,3.734,5.922,7.264,7.264,0,0,0-3.272,3.779A3.5,3.5,0,0,1,49.313,249.985Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M64.377,304.551l-.123,0a3.5,3.5,0,0,1-3.377-3.619c.319-9.243-3.608-14.043-3.648-14.089a3.5,3.5,0,0,1,5.29-4.584c.236.272,5.771,6.8,5.354,18.915A3.5,3.5,0,0,1,64.377,304.551Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M76,304.552a3.5,3.5,0,0,1-3.431-4.205c2.6-12.678.1-21.269.075-21.354a3.5,3.5,0,1,1,6.7-2.012c.125.418,3.033,10.387.076,24.774A3.5,3.5,0,0,1,76,304.552Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M86.5,304.535c-.081,0-.163,0-.245-.009a3.487,3.487,0,0,1-3.251-3.687,20.445,20.445,0,0,1,2.422-7.74,3.5,3.5,0,0,1,6.184,3.281,14.082,14.082,0,0,0-1.62,4.914A3.519,3.519,0,0,1,86.5,304.535Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M109.38,249.984a3.5,3.5,0,0,1-3.5-3.4c-.323-11.319-4.595-15.993-4.639-16.039a3.476,3.476,0,0,1,.019-4.93,3.52,3.52,0,0,1,4.968.019c.638.638,6.246,6.641,6.649,20.751a3.5,3.5,0,0,1-3.4,3.6Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M120.824,249.985a3.585,3.585,0,0,1-.58-.049,3.5,3.5,0,0,1-2.876-4.028,24.923,24.923,0,0,0-.134-8.55,3.5,3.5,0,0,1,6.888-1.25,31.882,31.882,0,0,1,.15,10.952A3.5,3.5,0,0,1,120.824,249.985Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M135.205,249.985a3.5,3.5,0,0,1-3.429-4.223c1.76-8.356,8.593-13.35,8.883-13.558a3.5,3.5,0,0,1,4.1,5.674c-.1.072-4.947,3.706-6.131,9.328A3.5,3.5,0,0,1,135.205,249.985Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M147.26,304.551a3.5,3.5,0,0,1-3.5-3.5,26.881,26.881,0,0,0-8.17-19.3,3.5,3.5,0,0,1,4.576-5.3,34.015,34.015,0,0,1,10.594,24.593A3.5,3.5,0,0,1,147.26,304.551Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M160.953,304.551a3.752,3.752,0,0,1-.409-.023,3.5,3.5,0,0,1-3.072-3.881c.775-6.665-1.516-10.792-1.614-10.965a3.5,3.5,0,0,1,6.038-3.541c.147.248,3.59,6.183,2.529,15.315A3.5,3.5,0,0,1,160.953,304.551Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M171.021,304.551a3.547,3.547,0,0,1-.4-.022,3.5,3.5,0,0,1-3.085-3.87,21.984,21.984,0,0,1,6.049-12.263,3.5,3.5,0,0,1,4.838,5.059,14.909,14.909,0,0,0-3.931,7.989A3.5,3.5,0,0,1,171.021,304.551Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M257.168,304.551a3.5,3.5,0,0,1-3.5-3.5,34.015,34.015,0,0,1,10.594-24.593,3.5,3.5,0,0,1,4.573,5.3,26.888,26.888,0,0,0-8.167,19.294A3.5,3.5,0,0,1,257.168,304.551Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M243.475,304.551A3.5,3.5,0,0,1,240,301.456c-1.062-9.132,2.381-15.067,2.529-15.315a3.5,3.5,0,0,1,6.035,3.545c-.124.221-2.381,4.338-1.611,10.961a3.5,3.5,0,0,1-3.072,3.881A3.766,3.766,0,0,1,243.475,304.551Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M233.407,304.551a3.5,3.5,0,0,1-3.472-3.091,14.963,14.963,0,0,0-3.933-8,3.5,3.5,0,1,1,4.839-5.059,22,22,0,0,1,6.048,12.263,3.5,3.5,0,0,1-3.085,3.87A3.561,3.561,0,0,1,233.407,304.551Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M216.8,249.984a3.5,3.5,0,0,1-3.5-3.5,26.636,26.636,0,0,0-3.852-13.821,3.5,3.5,0,1,1,6.011-3.588,33.551,33.551,0,0,1,4.841,17.409A3.5,3.5,0,0,1,216.8,249.984Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M230.488,249.984a3.505,3.505,0,0,1-3.481-3.9c.775-6.665-1.515-10.793-1.613-10.965a3.5,3.5,0,0,1,6.039-3.541c.147.248,3.589,6.184,2.527,15.314A3.5,3.5,0,0,1,230.488,249.984Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M240.557,249.984a3.315,3.315,0,0,1-.4-.023,3.5,3.5,0,0,1-3.086-3.87,22.522,22.522,0,0,1,2.756-8.018,3.5,3.5,0,0,1,6.07,3.486,15.819,15.819,0,0,0-1.87,5.317A3.5,3.5,0,0,1,240.557,249.984Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M314.119,249.984c-.072,0-.144,0-.217-.007a3.5,3.5,0,0,1-3.28-3.707c.626-10.25-2.385-16.139-2.415-16.2a3.5,3.5,0,0,1,6.167-3.311c.164.3,3.992,7.536,3.235,19.935A3.5,3.5,0,0,1,314.119,249.984Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M303.369,249.984a3.5,3.5,0,0,1-3.5-3.5c0-3.585-3.055-6.329-3.085-6.355a3.5,3.5,0,1,1,4.519-5.345c.228.192,5.566,4.785,5.566,11.7A3.5,3.5,0,0,1,303.369,249.984Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M303.7,304.551c-.095,0-.189,0-.285-.011a3.5,3.5,0,0,1-3.208-3.77,40.04,40.04,0,0,0-2.757-17.206,3.5,3.5,0,0,1,6.4-2.837,46.563,46.563,0,0,1,3.336,20.605A3.5,3.5,0,0,1,303.7,304.551Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M314.4,304.552a3.5,3.5,0,0,1-3.061-5.192c2.48-4.5,1.971-11.056,1.965-11.121a3.5,3.5,0,1,1,6.972-.627c.032.352.736,8.708-2.808,15.131A3.5,3.5,0,0,1,314.4,304.552Z"
                            fill="#abb737"
                        ></path>
                        <path
                            d="M236.584,158.031a44.341,44.341,0,0,0-43.385-6.684l-17.115,6.539A43.51,43.51,0,0,1,130,148.219L97.922,116.576A38.429,38.429,0,0,0,47.31,113.65c9.153-2.3,27.109-3.348,42.891,16.38l26.977,31.463s18.639,29.166,65.1,10.362L204.65,162.6s17.536-7.048,35.459-1.772Z"
                            fill="#d9cc66"
                        ></path>
                    </g>
                </svg>
            );
        case "marker":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 1000.80 1000.80"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g strokeWidth="0"></g>
                    <g
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        stroke="#000000"
                        strokeWidth="67.584"
                    >
                        <path
                            d="M512 85.333333c-164.949333 0-298.666667 133.738667-298.666667 298.666667 0 164.949333 298.666667 554.666667 298.666667 554.666667s298.666667-389.717333 298.666667-554.666667c0-164.928-133.717333-298.666667-298.666667-298.666667z m0 448a149.333333 149.333333 0 1 1 0-298.666666 149.333333 149.333333 0 0 1 0 298.666666z"
                            fill="#FF3D00"
                            className={className}
                        ></path>
                    </g>
                    <g>
                        <path
                            d="M512 85.333333c-164.949333 0-298.666667 133.738667-298.666667 298.666667 0 164.949333 298.666667 554.666667 298.666667 554.666667s298.666667-389.717333 298.666667-554.666667c0-164.928-133.717333-298.666667-298.666667-298.666667z m0 448a149.333333 149.333333 0 1 1 0-298.666666 149.333333 149.333333 0 0 1 0 298.666666z"
                            fill="#FF3D00"
                            className={className}
                        ></path>
                    </g>
                </svg>
            );
        case "logo-text":
            return (
                <svg
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 508 114"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M126.16 41.92L92.7 13C91.05 11.57 88.59 11.57 86.94 13L61.64 34.87C60.67 35.71 60.12 36.92 60.12 38.2V69.21H29.03C26.6 69.21 24.62 71.18 24.62 73.62V106.43H7.48004V46.54L51.07 8.85L63.94 20.63L69.82 15.54L53.94 1.82C52.29 0.389998 49.83 0.389998 48.18 1.82L1.79004 41.92C0.820039 42.76 0.27002 43.97 0.27002 45.25V109.21C0.27002 111.64 2.24002 113.62 4.68002 113.62H123.29C125.72 113.62 127.7 111.65 127.7 109.21V45.25C127.69 43.98 127.13 42.76 126.16 41.92ZM97.21 106.43H31.81V76.42H97.21V106.43ZM120.48 106.43H104.41V73.62C104.41 71.19 102.44 69.21 100 69.21H67.31V39.48L89.82 20.02L120.49 46.53V106.43H120.48Z"
                        fill="black"
                        className={className}
                    />
                    <path
                        d="M504.57 78.94C502.45 75.52 499.63 72.81 496.1 70.83C492.57 68.85 488.63 67.86 484.3 67.86C479.97 67.86 476.02 68.85 472.46 70.83C468.9 72.81 466.07 75.51 463.95 78.94C461.83 82.37 460.78 86.31 460.78 90.74C460.78 95.13 461.83 99.05 463.95 102.51C466.07 105.96 468.9 108.67 472.46 110.65C476.02 112.63 479.96 113.63 484.3 113.63C488.64 113.63 492.57 112.63 496.1 110.65C499.63 108.67 502.45 105.96 504.57 102.51C506.68 99.05 507.74 95.13 507.74 90.74C507.74 86.31 506.68 82.38 504.57 78.94ZM493.89 97.01C492.96 98.83 491.67 100.25 490.04 101.26C488.41 102.28 486.5 102.78 484.3 102.78C482.11 102.78 480.18 102.28 478.52 101.26C476.86 100.25 475.56 98.82 474.63 97.01C473.69 95.19 473.22 93.1 473.22 90.75C473.22 88.39 473.69 86.32 474.63 84.53C475.56 82.73 476.86 81.32 478.52 80.28C480.18 79.23 482.11 78.71 484.3 78.71C486.5 78.71 488.41 79.23 490.04 80.28C491.67 81.32 492.96 82.74 493.89 84.53C494.83 86.32 495.3 88.39 495.3 90.75C495.29 93.1 494.83 95.19 493.89 97.01ZM458.05 68.83L442.15 86.5L457.97 112.67H444.32L433.18 93.97L428.34 99.25V112.67H416.3V51.9H428.34V84.61L443.19 68.83H458.05ZM347.33 68.83H359.37V112.67H347.33V68.83ZM329.03 68.83H342.04L324.78 112.67H314.35L297.09 68.83H310.1L319.58 95.09L329.03 68.83ZM347.33 52.85H359.37V64.9H347.33V52.85ZM408.35 84.49V112.67H396.3V87.06C396.3 85.34 395.96 83.87 395.3 82.64C394.63 81.41 393.69 80.45 392.49 79.75C391.28 79.06 389.89 78.71 388.28 78.71C386.73 78.71 385.33 79.06 384.1 79.75C382.87 80.45 381.92 81.41 381.25 82.64C380.58 83.87 380.25 85.34 380.25 87.06V112.67H368.2V68.83H379.44V74.19C380.44 72.26 381.89 70.78 383.74 69.76C386.01 68.5 388.68 67.87 391.73 67.87C395.04 67.87 397.95 68.56 400.44 69.96C402.93 71.35 404.87 73.28 406.26 75.78C407.65 78.27 408.35 81.17 408.35 84.49Z"
                        fill="black"
                        className={className}
                    />
                    <path
                        d="M153.69 52.85H165.73V64.9H153.69V52.85ZM153.69 112.67H165.73V68.83H153.69V112.67ZM233.37 69.96C230.93 68.56 228.14 67.87 224.98 67.87C221.39 67.87 218.26 68.73 215.59 70.44C213.99 71.46 212.61 72.82 211.45 74.52C210.55 73.01 209.29 71.72 207.68 70.64C204.92 68.8 201.73 67.87 198.09 67.87C194.67 67.87 191.72 68.67 189.26 70.28C187.78 71.25 186.63 72.53 185.81 74.12V68.82H174.56V112.66H186.61V87.05C186.61 85.33 186.91 83.86 187.53 82.63C188.14 81.4 189.03 80.44 190.18 79.74C191.33 79.05 192.68 78.7 194.23 78.7C195.84 78.7 197.2 79.05 198.33 79.74C199.45 80.44 200.32 81.4 200.93 82.63C201.55 83.86 201.85 85.33 201.85 87.05V112.66H213.9V87.05C213.9 85.33 214.2 83.86 214.82 82.63C215.43 81.4 216.32 80.44 217.47 79.74C218.62 79.05 219.97 78.7 221.52 78.7C223.13 78.7 224.49 79.05 225.61 79.74C226.74 80.44 227.61 81.4 228.22 82.63C228.84 83.86 229.14 85.33 229.14 87.05V112.66H241.19V84.48C241.19 81.17 240.49 78.26 239.1 75.77C237.72 73.29 235.81 71.35 233.37 69.96ZM294.35 90.75C294.35 95.14 293.29 99.06 291.18 102.52C289.06 105.97 286.24 108.68 282.71 110.66C279.18 112.64 275.24 113.64 270.91 113.64C266.58 113.64 262.63 112.64 259.07 110.66C255.51 108.68 252.67 105.97 250.56 102.52C248.44 99.06 247.39 95.14 247.39 90.75C247.39 86.31 248.44 82.38 250.56 78.95C252.67 75.53 255.51 72.82 259.07 70.84C262.63 68.86 266.57 67.87 270.91 67.87C275.25 67.87 279.18 68.86 282.71 70.84C286.24 72.82 289.06 75.52 291.18 78.95C293.29 82.38 294.35 86.31 294.35 90.75ZM281.91 90.75C281.91 88.39 281.44 86.32 280.5 84.53C279.56 82.73 278.28 81.32 276.65 80.28C275.02 79.23 273.1 78.71 270.91 78.71C268.72 78.71 266.79 79.23 265.13 80.28C263.47 81.32 262.17 82.74 261.24 84.53C260.3 86.32 259.83 88.39 259.83 90.75C259.83 93.11 260.3 95.2 261.24 97.01C262.17 98.83 263.47 100.25 265.13 101.26C266.79 102.28 268.72 102.78 270.91 102.78C273.1 102.78 275.02 102.28 276.65 101.26C278.28 100.25 279.56 98.82 280.5 97.01C281.44 95.2 281.91 93.11 281.91 90.75Z"
                        fill="black"
                        className={className}
                    />
                </svg>
            );
        case "logo":
            return (
                <svg
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 99 88"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        className={className}
                        d="M97.63 32.46L71.88 10.2C70.61 9.1 68.72 9.1 67.45 10.2L47.98 27.03C47.24 27.67 46.81 28.61 46.81 29.6V53.46H22.89C21.02 53.46 19.5 54.98 19.5 56.85V82.09H6.30002V36.01L39.84 7.01L49.75 16.07L54.28 12.16L42.06 1.59999C40.79 0.499994 38.9 0.499994 37.63 1.59999L1.94002 32.46C1.20002 33.1 0.77002 34.04 0.77002 35.03V84.25C0.77002 86.12 2.29002 87.64 4.16002 87.64H95.42C97.29 87.64 98.81 86.12 98.81 84.25V35.03C98.8 34.04 98.37 33.11 97.63 32.46ZM75.35 82.1H25.03V59H75.35V82.1ZM93.26 82.1H80.89V56.86C80.89 54.99 79.37 53.47 77.5 53.47H52.34V30.6L69.66 15.63L93.26 36.03V82.1Z"
                        fill="black"
                    />
                </svg>
            );
        case "image-plus":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <rect x="0" fill="none" width="24" height="24" />

                    <g>
                        <path d="M23 4v2h-3v3h-2V6h-3V4h3V1h2v3h3zm-8.5 7c.828 0 1.5-.672 1.5-1.5S15.328 8 14.5 8 13 8.672 13 9.5s.672 1.5 1.5 1.5zm3.5 3.234l-.513-.57c-.794-.885-2.18-.885-2.976 0l-.655.73L9 9l-3 3.333V6h7V4H6c-1.105 0-2 .895-2 2v12c0 1.105.895 2 2 2h12c1.105 0 2-.895 2-2v-7h-2v3.234z" />
                    </g>
                </svg>
            );
        case "heart":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            className={className}
                            d="M4.45067 13.9082L11.4033 20.4395C11.6428 20.6644 11.7625 20.7769 11.9037 20.8046C11.9673 20.8171 12.0327 20.8171 12.0963 20.8046C12.2375 20.7769 12.3572 20.6644 12.5967 20.4395L19.5493 13.9082C21.5055 12.0706 21.743 9.0466 20.0978 6.92607L19.7885 6.52734C17.8203 3.99058 13.8696 4.41601 12.4867 7.31365C12.2913 7.72296 11.7087 7.72296 11.5133 7.31365C10.1304 4.41601 6.17972 3.99058 4.21154 6.52735L3.90219 6.92607C2.25695 9.0466 2.4945 12.0706 4.45067 13.9082Z"
                            stroke="#33363F"
                            strokeWidth="3"
                        ></path>
                    </g>
                </svg>
            );
        case "close":
            return (
                <svg
                    width={width || DEFAULT_W}
                    className={className}
                    height={height || DEFAULT_H}
                    viewBox="0 0 1024 1024"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            className={className}
                            fill="#000000"
                            d="M195.2 195.2a64 64 0 0 1 90.496 0L512 421.504 738.304 195.2a64 64 0 0 1 90.496 90.496L602.496 512 828.8 738.304a64 64 0 0 1-90.496 90.496L512 602.496 285.696 828.8a64 64 0 0 1-90.496-90.496L421.504 512 195.2 285.696a64 64 0 0 1 0-90.496z"
                        ></path>
                    </g>
                </svg>
            );
        case "left":
            return (
                <svg
                    width={width || DEFAULT_W}
                    className={className}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M15.7071 4.29289C16.0976 4.68342 16.0976 5.31658 15.7071 5.70711L9.41421 12L15.7071 18.2929C16.0976 18.6834 16.0976 19.3166 15.7071 19.7071C15.3166 20.0976 14.6834 20.0976 14.2929 19.7071L7.29289 12.7071C7.10536 12.5196 7 12.2652 7 12C7 11.7348 7.10536 11.4804 7.29289 11.2929L14.2929 4.29289C14.6834 3.90237 15.3166 3.90237 15.7071 4.29289Z"
                            fill="#000000"
                        ></path>
                    </g>
                </svg>
            );
        case "right":
            return (
                <svg
                    width={width || DEFAULT_W}
                    className={className}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M8.29289 4.29289C8.68342 3.90237 9.31658 3.90237 9.70711 4.29289L16.7071 11.2929C17.0976 11.6834 17.0976 12.3166 16.7071 12.7071L9.70711 19.7071C9.31658 20.0976 8.68342 20.0976 8.29289 19.7071C7.90237 19.3166 7.90237 18.6834 8.29289 18.2929L14.5858 12L8.29289 5.70711C7.90237 5.31658 7.90237 4.68342 8.29289 4.29289Z"
                            fill="#000000"
                        ></path>
                    </g>
                </svg>
            );
        case "area":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    className={className}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path d="M3 5v14c0 1.103.897 2 2 2h14c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2H5c-1.103 0-2 .897-2 2zm16.002 14H5V5h14l.002 14z"></path>
                        <path d="M15 12h2V7h-5v2h3zm-3 3H9v-3H7v5h5z"></path>
                    </g>
                </svg>
            );
        case "bed":
            return (
                <svg
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 32 32"
                    enableBackground="new 0 0 32 32"
                    width={width || DEFAULT_W}
                    className={className}
                    height={height || DEFAULT_H}
                    fill="#000000"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            fill="none"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeMiterlimit="10"
                            d="M28,16V9c0-1.1-0.9-2-2-2H6C4.9,7,4,7.9,4,9v7"
                        ></path>
                        <path
                            fill="none"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeMiterlimit="10"
                            d="M8,16v-2c0-1.1,0.9-2,2-2h4c1.1,0,2,0.9,2,2v2"
                        ></path>
                        <path
                            fill="none"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeMiterlimit="10"
                            d="M16,16v-2c0-1.1,0.9-2,2-2h4c1.1,0,2,0.9,2,2v2"
                        ></path>
                        <path
                            fill="none"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeMiterlimit="10"
                            d="M3,18v8h3v-2h20v2h3v-8c0-1.1-0.9-2-2-2H5 C3.9,16,3,16.9,3,18z"
                        ></path>
                    </g>
                </svg>
            );
        case "bath":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    className={className}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path d="M22,12H5V5A2,2,0,0,1,7,3H9a2,2,0,0,1,1.838,1.214A3.5,3.5,0,0,0,8.5,7.5V9a1,1,0,0,0,1,1h5a1,1,0,0,0,1-1V7.5a3.5,3.5,0,0,0-2.6-3.368A4,4,0,0,0,9,1H7A4,4,0,0,0,3,5v7H2a1,1,0,0,0,0,2H3v4a3,3,0,0,0,2,2.816V22a1,1,0,0,0,2,0V21H17v1a1,1,0,0,0,2,0V20.816A3,3,0,0,0,21,18V14h1a1,1,0,0,0,0-2ZM13.5,7.5V8h-3V7.5a1.5,1.5,0,0,1,3,0ZM19,18a1,1,0,0,1-1,1H6a1,1,0,0,1-1-1V14H19Z"></path>
                    </g>
                </svg>
            );
        case "car":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    className={className}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path d="M20,10h-.341L17.18,4.214A2,2,0,0,0,15.341,3H4.754A2.008,2.008,0,0,0,2.831,4.451L1.038,10.726a1.019,1.019,0,0,0-.005.111A.889.889,0,0,0,1,11v5a2,2,0,0,0,2,2h.142a3.981,3.981,0,0,0,7.716,0h2.284a3.981,3.981,0,0,0,7.716,0H21a2,2,0,0,0,2-2V13A3,3,0,0,0,20,10ZM12,5h3.341l2.142,5H12ZM4.754,5H10v5H3.325ZM7,19a2,2,0,1,1,2-2A2,2,0,0,1,7,19Zm10,0a2,2,0,1,1,2-2A2,2,0,0,1,17,19Zm4-3h-.142a3.981,3.981,0,0,0-7.716,0H10.858a3.981,3.981,0,0,0-7.716,0H3V12H20a1,1,0,0,1,1,1Z"></path>
                    </g>
                </svg>
            );
        case "show-more":
            return (
                <svg
                    width={width || DEFAULT_W}
                    className={className}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            d="M4 18.005C4 19.1078 4.9 20 6 20C7.1 20 8 19.1078 8 18.005C8 16.9023 7.1 16 6 16C4.9 16 4 16.9023 4 18.005Z"
                            fill="#000000"
                        ></path>
                        <path
                            d="M4 12.005C4 13.1078 4.9 14 6 14C7.1 14 8 13.1078 8 12.005C8 10.9023 7.1 10 6 10C4.9 10 4 10.9023 4 12.005Z"
                            fill="#000000"
                        ></path>
                        <path
                            d="M4 6.00501C4 7.10777 4.9 8 6 8C7.1 8 8 7.10777 8 6.00501C8 4.90226 7.1 4 6 4C4.9 4 4 4.90226 4 6.00501Z"
                            fill="#000000"
                        ></path>
                        <path
                            d="M10 6.00501C10 7.10777 10.9 8 12 8C13.1 8 14 7.10777 14 6.00501C14 4.90226 13.1 4 12 4C10.9 4 10 4.90226 10 6.00501Z"
                            fill="#000000"
                        ></path>
                        <path
                            d="M10 12.005C10 13.1078 10.9 14 12 14C13.1 14 14 13.1078 14 12.005C14 10.9023 13.1 10 12 10C10.9 10 10 10.9023 10 12.005Z"
                            fill="#000000"
                        ></path>
                        <path
                            d="M10 18.005C10 19.1078 10.9 20 12 20C13.1 20 14 19.1078 14 18.005C14 16.9023 13.1 16 12 16C10.9 16 10 16.9023 10 18.005Z"
                            fill="#000000"
                        ></path>
                        <path
                            d="M16 12.005C16 13.1078 16.9 14 18 14C19.1 14 20 13.1078 20 12.005C20 10.9023 19.1 10 18 10C16.9 10 16 10.9023 16 12.005Z"
                            fill="#000000"
                        ></path>
                        <path
                            d="M16 18.005C16 19.1078 16.9 20 18 20C19.1 20 20 19.1078 20 18.005C20 16.9023 19.1 16 18 16C16.9 16 16 16.9023 16 18.005Z"
                            fill="#000000"
                        ></path>
                        <path
                            d="M16 6.00501C16 7.10777 16.9 8 18 8C19.1 8 20 7.10777 20 6.00501C20 4.90226 19.1 4 18 4C16.9 4 16 4.90226 16 6.00501Z"
                            fill="#000000"
                        ></path>
                    </g>
                </svg>
            );
        case "list":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className={className}
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <g>
                            <path
                                d="M9 17H19M9 12H19M9 7H19M5.00195 17V17.002L5 17.002V17H5.00195ZM5.00195 12V12.002L5 12.002V12H5.00195ZM5.00195 7V7.002L5 7.00195V7H5.00195Z"
                                stroke="#000000"
                                strokeWidth="2"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                                className={className}
                            ></path>
                        </g>
                    </g>
                </svg>
            );
        case "cards":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <rect
                            x="3"
                            y="3"
                            width="7"
                            height="7"
                            rx="1"
                            stroke="#222222"
                            strokeLinecap="round"
                        ></rect>
                        <rect
                            x="3"
                            y="14"
                            width="7"
                            height="7"
                            rx="1"
                            stroke="#222222"
                            strokeLinecap="round"
                        ></rect>
                        <rect
                            x="14"
                            y="3"
                            width="7"
                            height="7"
                            rx="1"
                            stroke="#222222"
                            strokeLinecap="round"
                        ></rect>
                        <rect
                            x="14"
                            y="14"
                            width="7"
                            height="7"
                            rx="1"
                            stroke="#222222"
                            strokeLinecap="round"
                        ></rect>
                    </g>
                </svg>
            );
        case "office":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className={className}
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            className={className}
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M5 4C4.44772 4 4 4.44772 4 5V19C4 19.5523 4.44772 20 5 20H12H13C13.5523 20 14 19.5523 14 19V5C14 4.44772 13.5523 4 13 4H5ZM5 22H12H13H19C20.6569 22 22 20.6569 22 19V9C22 7.34315 20.6569 6 19 6H16V5C16 3.34315 14.6569 2 13 2H5C3.34315 2 2 3.34315 2 5V19C2 20.6569 3.34315 22 5 22ZM19 20H15.8293C15.9398 19.6872 16 19.3506 16 19V8H19C19.5523 8 20 8.44772 20 9V19C20 19.5523 19.5523 20 19 20ZM7 14H5V16H7V14ZM8 14H10V16H8V14ZM13 14H11V16H13V14ZM17 14H19V16H17V14ZM19 10H17V12H19V10ZM5 10H7V12H5V10ZM10 10H8V12H10V10ZM11 10H13V12H11V10ZM7 6H5V8H7V6ZM8 6H10V8H8V6ZM13 6H11V8H13V6Z"
                            fill="#ffffff"
                        ></path>
                    </g>
                </svg>
            );
        case "account-settings":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                    className={className}
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            className={className}
                            d="M9.6,3.32a3.86,3.86,0,1,0,3.86,3.85A3.85,3.85,0,0,0,9.6,3.32M16.35,11a.26.26,0,0,0-.25.21l-.18,1.27a4.63,4.63,0,0,0-.82.45l-1.2-.48a.3.3,0,0,0-.3.13l-1,1.66a.24.24,0,0,0,.06.31l1,.79a3.94,3.94,0,0,0,0,1l-1,.79a.23.23,0,0,0-.06.3l1,1.67c.06.13.19.13.3.13l1.2-.49a3.85,3.85,0,0,0,.82.46l.18,1.27a.24.24,0,0,0,.25.2h1.93a.24.24,0,0,0,.23-.2l.18-1.27a5,5,0,0,0,.81-.46l1.19.49c.12,0,.25,0,.32-.13l1-1.67a.23.23,0,0,0-.06-.3l-1-.79a4,4,0,0,0,0-.49,2.67,2.67,0,0,0,0-.48l1-.79a.25.25,0,0,0,.06-.31l-1-1.66c-.06-.13-.19-.13-.31-.13L19.5,13a4.07,4.07,0,0,0-.82-.45l-.18-1.27a.23.23,0,0,0-.22-.21H16.46M9.71,13C5.45,13,2,14.7,2,16.83v1.92h9.33a6.65,6.65,0,0,1,0-5.69A13.56,13.56,0,0,0,9.71,13m7.6,1.43a1.45,1.45,0,1,1,0,2.89,1.45,1.45,0,0,1,0-2.89Z"
                        ></path>
                    </g>
                </svg>
            );
        case "property":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            d="M22 22L2 22"
                            stroke="#1C274C"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            className={className}
                        ></path>
                        <path
                            d="M2 11L6.06296 7.74968M22 11L13.8741 4.49931C12.7784 3.62279 11.2216 3.62279 10.1259 4.49931L9.34398 5.12486"
                            stroke="#1C274C"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            className={className}
                        ></path>
                        <path
                            d="M15.5 5.5V3.5C15.5 3.22386 15.7239 3 16 3H18.5C18.7761 3 19 3.22386 19 3.5V8.5"
                            stroke="#1C274C"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            className={className}
                        ></path>
                        <path
                            d="M4 22V9.5"
                            stroke="#1C274C"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            className={className}
                        ></path>
                        <path
                            d="M20 9.5V13.5M20 22V17.5"
                            stroke="#1C274C"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            className={className}
                        ></path>
                        <path
                            d="M15 22V17C15 15.5858 15 14.8787 14.5607 14.4393C14.1213 14 13.4142 14 12 14C10.5858 14 9.87868 14 9.43934 14.4393M9 22V17"
                            stroke="#1C274C"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            className={className}
                        ></path>
                        <path
                            d="M14 9.5C14 10.6046 13.1046 11.5 12 11.5C10.8954 11.5 10 10.6046 10 9.5C10 8.39543 10.8954 7.5 12 7.5C13.1046 7.5 14 8.39543 14 9.5Z"
                            stroke="#1C274C"
                            strokeWidth="1.5"
                            className={className}
                        ></path>
                    </g>
                </svg>
            );
        case "account-plus":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <g>
                            <g>
                                <g>
                                    <path
                                        d="M17,21V19a4,4,0,0,0-4-4H5a4,4,0,0,0-4,4v2"
                                        fill="none"
                                        stroke="#000000"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        strokeWidth="2"
                                        className={className}
                                    ></path>
                                    <circle
                                        cx="9"
                                        cy="7"
                                        fill="none"
                                        r="4"
                                        stroke="#000000"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        strokeWidth="2"
                                        className={className}
                                    ></circle>
                                    <line
                                        fill="none"
                                        stroke="#000000"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        strokeWidth="2"
                                        x1="17"
                                        x2="23"
                                        y1="11"
                                        y2="11"
                                        className={className}
                                    ></line>
                                    <line
                                        fill="none"
                                        stroke="#000000"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        strokeWidth="2"
                                        x1="20"
                                        x2="20"
                                        y1="8"
                                        y2="14"
                                        className={className}
                                    ></line>
                                </g>
                            </g>
                        </g>
                    </g>
                </svg>
            );
        case "filter":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            className={className}
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M3 7C3 6.44772 3.44772 6 4 6H20C20.5523 6 21 6.44772 21 7C21 7.55228 20.5523 8 20 8H4C3.44772 8 3 7.55228 3 7ZM6 12C6 11.4477 6.44772 11 7 11H17C17.5523 11 18 11.4477 18 12C18 12.5523 17.5523 13 17 13H7C6.44772 13 6 12.5523 6 12ZM9 17C9 16.4477 9.44772 16 10 16H14C14.5523 16 15 16.4477 15 17C15 17.5523 14.5523 18 14 18H10C9.44772 18 9 17.5523 9 17Z"
                            fill="#000000"
                        ></path>
                    </g>
                </svg>
            );
        case "checkmark":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            className={className}
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M21.2287 6.60355C21.6193 6.99407 21.6193 7.62723 21.2287 8.01776L10.2559 18.9906C9.86788 19.3786 9.23962 19.3814 8.84811 18.9969L2.66257 12.9218C2.26855 12.5349 2.26284 11.9017 2.64983 11.5077L3.35054 10.7942C3.73753 10.4002 4.37067 10.3945 4.7647 10.7815L9.53613 15.4677L19.1074 5.89644C19.4979 5.50592 20.1311 5.50591 20.5216 5.89644L21.2287 6.60355Z"
                            fill="#000000"
                        ></path>
                    </g>
                </svg>
            );
        case "notification":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g strokeWidth="0"></g>
                    <g strokeLinecap="round" strokeLinejoin="round"></g>
                    <g>
                        <path
                            className={className}
                            d="M17.7778 16.4066C18.1725 16.2808 18.3904 15.8588 18.2646 15.4642C18.1388 15.0696 17.7168 14.8516 17.3222 14.9774L17.7778 16.4066ZM11.6797 15.9343C11.2657 15.9455 10.9391 16.2902 10.9503 16.7043C10.9615 17.1183 11.3062 17.4449 11.7203 17.4337L11.6797 15.9343ZM16.876 16.0131C17.0578 16.3853 17.5068 16.5397 17.8791 16.358C18.2513 16.1762 18.4057 15.7272 18.224 15.3549L16.876 16.0131ZM16.9104 14.374L16.2225 14.6728C16.2269 14.683 16.2316 14.6931 16.2364 14.7031L16.9104 14.374ZM15.9422 10.806L16.6868 10.7157C16.6855 10.7048 16.6839 10.694 16.6821 10.6832L15.9422 10.806ZM11.7068 5.941C11.2926 5.941 10.9568 6.27679 10.9568 6.691C10.9568 7.10521 11.2926 7.441 11.7068 7.441V5.941ZM11.7068 7.441C12.121 7.441 12.4568 7.10521 12.4568 6.691C12.4568 6.27679 12.121 5.941 11.7068 5.941V7.441ZM7.45778 10.807L6.71795 10.6839C6.71614 10.6948 6.71457 10.7057 6.71325 10.7166L7.45778 10.807ZM6.48961 14.374L7.16357 14.7031C7.16843 14.6931 7.17308 14.683 7.1775 14.6728L6.48961 14.374ZM5.17605 15.3549C4.99431 15.7272 5.14873 16.1762 5.52095 16.358C5.89317 16.5397 6.34223 16.3853 6.52397 16.0131L5.17605 15.3549ZM10.9568 6.691C10.9568 7.10521 11.2926 7.441 11.7068 7.441C12.121 7.441 12.4568 7.10521 12.4568 6.691H10.9568ZM12.4568 5C12.4568 4.58579 12.121 4.25 11.7068 4.25C11.2926 4.25 10.9568 4.58579 10.9568 5H12.4568ZM10.9568 6.691C10.9568 7.10521 11.2926 7.441 11.7068 7.441C12.121 7.441 12.4568 7.10521 12.4568 6.691H10.9568ZM12.4568 5C12.4568 4.58579 12.121 4.25 11.7068 4.25C11.2926 4.25 10.9568 4.58579 10.9568 5H12.4568ZM6.07781 14.9774C5.68317 14.8516 5.26125 15.0696 5.13544 15.4642C5.00963 15.8588 5.22756 16.2808 5.6222 16.4066L6.07781 14.9774ZM11.6797 17.4337C12.0938 17.4449 12.4385 17.1183 12.4497 16.7043C12.4609 16.2902 12.1343 15.9455 11.7203 15.9343L11.6797 17.4337ZM10.5 16.523C10.5 16.1088 10.1642 15.773 9.75001 15.773C9.33579 15.773 9.00001 16.1088 9.00001 16.523H10.5ZM14.4049 16.523C14.4049 16.1088 14.0691 15.773 13.6549 15.773C13.2407 15.773 12.9049 16.1088 12.9049 16.523H14.4049ZM17.3222 14.9774C15.4929 15.5606 13.5943 15.8825 11.6797 15.9343L11.7203 17.4337C13.7761 17.3782 15.8144 17.0325 17.7778 16.4066L17.3222 14.9774ZM18.224 15.3549L17.5844 14.0449L16.2364 14.7031L16.876 16.0131L18.224 15.3549ZM17.5983 14.0752C17.1347 13.008 16.8275 11.8759 16.6868 10.7157L15.1977 10.8963C15.3558 12.1997 15.701 13.4723 16.2225 14.6728L17.5983 14.0752ZM16.6821 10.6832C16.4696 9.40313 16.017 8.21501 15.1948 7.33897C14.3526 6.44171 13.1869 5.941 11.7068 5.941V7.441C12.8193 7.441 13.5729 7.80279 14.1011 8.36553C14.6492 8.94949 15.0181 9.81887 15.2024 10.9288L16.6821 10.6832ZM11.7068 5.941C10.2266 5.941 9.05835 6.44146 8.21321 7.33789C7.38755 8.21366 6.93123 9.40201 6.71795 10.6839L8.19761 10.9301C8.38214 9.82099 8.75359 8.95134 9.30463 8.36686C9.83619 7.80304 10.5936 7.441 11.7068 7.441V5.941ZM6.71325 10.7166C6.57244 11.8765 6.26519 13.0083 5.80171 14.0752L7.1775 14.6728C7.69888 13.4727 8.04413 12.2004 8.20232 10.8974L6.71325 10.7166ZM5.81565 14.0449L5.17605 15.3549L6.52397 16.0131L7.16357 14.7031L5.81565 14.0449ZM12.4568 6.691V5H10.9568V6.691H12.4568ZM12.4568 6.691V5H10.9568V6.691H12.4568ZM5.6222 16.4066C7.58558 17.0325 9.62394 17.3782 11.6797 17.4337L11.7203 15.9343C9.80576 15.8825 7.90713 15.5606 6.07781 14.9774L5.6222 16.4066ZM9.00001 16.523V17.023H10.5V16.523H9.00001ZM9.00001 17.023C9.00001 18.5252 10.1921 19.7755 11.7024 19.7755V18.2755C11.0561 18.2755 10.5 17.7327 10.5 17.023H9.00001ZM11.7024 19.7755C13.2127 19.7755 14.4049 18.5252 14.4049 17.023H12.9049C12.9049 17.7327 12.3487 18.2755 11.7024 18.2755V19.7755ZM14.4049 17.023V16.523H12.9049V17.023H14.4049Z"
                            fill="#000000"
                        ></path>
                    </g>
                </svg>
            );
        case "success":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            fillRule="evenodd"
                            d="M12,2 C17.5228475,2 22,6.4771525 22,12 C22,17.5228475 17.5228475,22 12,22 C6.4771525,22 2,17.5228475 2,12 C2,6.4771525 6.4771525,2 12,2 Z M12,4 C7.581722,4 4,7.581722 4,12 C4,16.418278 7.581722,20 12,20 C16.418278,20 20,16.418278 20,12 C20,7.581722 16.418278,4 12,4 Z M15.2928932,8.29289322 L10,13.5857864 L8.70710678,12.2928932 C8.31658249,11.9023689 7.68341751,11.9023689 7.29289322,12.2928932 C6.90236893,12.6834175 6.90236893,13.3165825 7.29289322,13.7071068 L9.29289322,15.7071068 C9.68341751,16.0976311 10.3165825,16.0976311 10.7071068,15.7071068 L16.7071068,9.70710678 C17.0976311,9.31658249 17.0976311,8.68341751 16.7071068,8.29289322 C16.3165825,7.90236893 15.6834175,7.90236893 15.2928932,8.29289322 Z"
                        ></path>
                    </g>
                </svg>
            );
        case "solo-arrow":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            d="M6 18L18 6M18 6H9M18 6V15"
                            stroke="#000"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                    </g>
                </svg>
            );
        case "missing-image":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            d="M14.2647 15.9377L12.5473 14.2346C11.758 13.4519 11.3633 13.0605 10.9089 12.9137C10.5092 12.7845 10.079 12.7845 9.67922 12.9137C9.22485 13.0605 8.83017 13.4519 8.04082 14.2346L4.04193 18.2622M14.2647 15.9377L14.606 15.5991C15.412 14.7999 15.8149 14.4003 16.2773 14.2545C16.6839 14.1262 17.1208 14.1312 17.5244 14.2688C17.9832 14.4253 18.3769 14.834 19.1642 15.6515L20 16.5001M14.2647 15.9377L18.22 19.9628M12 4H7.2C6.07989 4 5.51984 4 5.09202 4.21799C4.7157 4.40973 4.40973 4.71569 4.21799 5.09202C4 5.51984 4 6.0799 4 7.2V16.8C4 17.4466 4 17.9066 4.04193 18.2622M4.04193 18.2622C4.07264 18.5226 4.12583 18.7271 4.21799 18.908C4.40973 19.2843 4.7157 19.5903 5.09202 19.782C5.51984 20 6.07989 20 7.2 20H16.8C17.9201 20 18.4802 20 18.908 19.782C19.2843 19.5903 19.5903 19.2843 19.782 18.908C20 18.4802 20 17.9201 20 16.8V12M16 3L18.5 5.5M18.5 5.5L21 8M18.5 5.5L21 3M18.5 5.5L16 8"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                    </g>
                </svg>
            );
        case "down-chevron":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            d="M7 10L12 15L17 10"
                            className={className}
                            stroke="#000000"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                    </g>
                </svg>
            );
        case "traffic":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    version="1.1"
                    viewBox="0 0 301.397 301.397"
                >
                    <g>
                        <g>
                            <g>
                                <g>
                                    <path
                                        className={className}
                                        d="M287.089,93.141L272.96,18.68c-0.698-3.679-3.914-6.341-7.659-6.341H122.15c-3.745,0-6.96,2.662-7.659,6.342 l-12.02,63.344h15.868l10.265-54.096h79.431c-0.898,1.979-1.402,4.174-1.402,6.489c0,8.688,7.043,15.73,15.73,15.73 c8.688,0,15.73-7.043,15.73-15.73c0-2.315-0.504-4.51-1.402-6.489h22.153l11.18,58.918c-2.017-0.17-2.416-0.131-9.328-0.131 l-0.063-13.68c-0.049-9.855-8.107-17.872-17.962-17.872c-4.233,0-36.519,0-40.747,0c-9.855,0-17.912,8.017-17.961,17.872 l-0.044,9.519c3.163,0.723,6.073,2.166,8.535,4.161c3.753,3.041,6.462,7.365,7.416,12.391l13.238,69.758 c0.735,0.673,1.437,1.375,2.125,2.09h29.572v16.399c0,10.394,8.426,18.82,18.82,18.82c10.394,0,18.82-8.426,18.82-18.82v-16.401 h6.12c7.085,0,12.829-5.744,12.829-12.829v-37.132C301.397,109.514,295.75,99.362,287.089,93.141z M264.166,149.177 c-9.812,0-17.767-7.954-17.767-17.767c0-0.044,0.003-0.086,0.003-0.13c0.07-9.752,7.995-17.637,17.764-17.637 c9.812,0,17.767,7.954,17.767,17.767C281.933,141.155,274.033,149.177,264.166,149.177z"
                                    ></path>
                                    <path
                                        className={className}
                                        d="M201.035,176.027c-0.496-2.614-13.552-71.417-14.13-74.461c-0.698-3.679-3.914-6.341-7.659-6.341H36.095 c-3.745,0-6.96,2.662-7.659,6.342l-14.129,74.461C5.062,182.668,0,193.156,0,203.879v37.132c0,7.085,5.744,12.829,12.829,12.829 h6.12c0,6.818,0,9.688,0,16.399c0,10.394,8.426,18.82,18.82,18.82s18.82-8.426,18.82-18.82c0-6.707,0-9.574,0-16.399h102.165 c0,6.818,0,9.688,0,16.399c0,10.394,8.426,18.82,18.82,18.82s18.82-8.426,18.82-18.82c0-6.707,0-9.574,0-16.399h6.12 c7.084,0,12.829-5.743,12.829-12.829v-37.132C215.342,192.854,210.038,182.495,201.035,176.027z M37.231,232.063 c-9.866,0-17.767-8.02-17.767-17.767c0-9.812,7.954-17.767,17.767-17.767c9.769,0,17.694,7.885,17.764,17.638 c0,0.044,0.003,0.086,0.003,0.129C54.998,224.109,47.043,232.063,37.231,232.063z M156.619,138.048c-4.233,0-36.519,0-40.747,0 c-9.855,0-17.912,8.017-17.961,17.872l-0.063,13.68c-69.179-0.001-64.24-0.057-66.478,0.131l11.18-58.918h79.431 c-0.898,1.979-1.402,4.174-1.402,6.489c0,8.688,7.043,15.73,15.73,15.73c8.688,0,15.73-7.043,15.73-15.73 c0-2.315-0.504-4.51-1.402-6.489h22.153l11.18,58.918c-2.01-0.169-2.404-0.131-9.328-0.131l-0.063-13.68 C174.532,146.065,166.474,138.048,156.619,138.048z M178.111,232.063c-9.812,0-17.767-7.954-17.767-17.767 c0-0.044,0.003-0.086,0.003-0.13c0.07-9.752,7.995-17.637,17.764-17.637c9.812,0,17.767,7.954,17.767,17.767 C195.878,224.041,187.979,232.063,178.111,232.063z"
                                    ></path>
                                </g>
                            </g>
                        </g>
                    </g>
                </svg>
            );
        case "walking":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="-96 0 512 512"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            d="M208 96c26.5 0 48-21.5 48-48S234.5 0 208 0s-48 21.5-48 48 21.5 48 48 48zm94.5 149.1l-23.3-11.8-9.7-29.4c-14.7-44.6-55.7-75.8-102.2-75.9-36-.1-55.9 10.1-93.3 25.2-21.6 8.7-39.3 25.2-49.7 46.2L17.6 213c-7.8 15.8-1.5 35 14.2 42.9 15.6 7.9 34.6 1.5 42.5-14.3L81 228c3.5-7 9.3-12.5 16.5-15.4l26.8-10.8-15.2 60.7c-5.2 20.8.4 42.9 14.9 58.8l59.9 65.4c7.2 7.9 12.3 17.4 14.9 27.7l18.3 73.3c4.3 17.1 21.7 27.6 38.8 23.3 17.1-4.3 27.6-21.7 23.3-38.8l-22.2-89c-2.6-10.3-7.7-19.9-14.9-27.7l-45.5-49.7 17.2-68.7 5.5 16.5c5.3 16.1 16.7 29.4 31.7 37l23.3 11.8c15.6 7.9 34.6 1.5 42.5-14.3 7.7-15.7 1.4-35.1-14.3-43zM73.6 385.8c-3.2 8.1-8 15.4-14.2 21.5l-50 50.1c-12.5 12.5-12.5 32.8 0 45.3s32.7 12.5 45.2 0l59.4-59.4c6.1-6.1 10.9-13.4 14.2-21.5l13.5-33.8c-55.3-60.3-38.7-41.8-47.4-53.7l-20.7 51.5z"
                        ></path>
                    </g>
                </svg>
            );
        case "cycling":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="-0.5 0 32 32"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            d="M25.8 14.32c-0.64 0-1.24 0.12-1.84 0.32l-1.52-3 2.6-3.88c0.28-0.4 0.12-1.32-0.72-1.32h-3.32c-0.48 0-0.84 0.36-0.84 0.84s0.36 0.84 0.84 0.84h1.76l-1.76 2.64h-9.68c-0.4 0-0.68 0.36-0.68 0.36v0 0 0 0l-2.56 3.76c-0.72-0.36-1.56-0.56-2.44-0.56-3.12 0-5.64 2.52-5.64 5.64s2.52 5.64 5.68 5.64c2.8 0 5.16-2.080 5.56-4.8h4.6c0.48 0 0.68-0.4 0.68-0.4v0l4.8-7.2 1.16 2.2c-1.4 1.040-2.32 2.68-2.32 4.56 0 3.12 2.52 5.64 5.64 5.64s5.64-2.52 5.64-5.64-2.56-5.64-5.64-5.64zM15.92 18.32l-3.2-5.92h7.12l-3.92 5.92zM11.24 13.2l3.16 5.92h-3.2c-0.2-1.28-0.84-2.44-1.76-3.28l1.8-2.64zM8.52 17.24c0.48 0.52 0.84 1.16 1 1.88h-2.28l1.28-1.88zM5.68 23.88c-2.16 0-3.96-1.76-3.96-3.96s1.76-3.92 3.96-3.92c0.52 0 1 0.12 1.48 0.28l-2.16 3.2c-0.36 0.44-0.040 1.28 0.68 1.28v0 0h3.84c-0.4 1.8-2 3.12-3.84 3.12zM25.8 23.88c-2.16 0-3.96-1.76-3.96-3.96 0-1.2 0.56-2.32 1.44-3.040l1.8 3.44c0.2 0.32 0.68 0.56 1.16 0.36 0.4-0.16 0.56-0.72 0.36-1.12l-1.8-3.4c0.32-0.080 0.68-0.16 1.040-0.16 2.16 0 3.96 1.76 3.96 3.96s-1.8 3.92-4 3.92zM9.8 9.4h3.080c0.48 0 0.84-0.36 0.84-0.84s-0.36-0.84-0.84-0.84h-3.080c-0.48 0-0.84 0.36-0.84 0.84-0.040 0.44 0.36 0.84 0.84 0.84z"
                        ></path>
                    </g>
                </svg>
            );
        case "location":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            d="M5.36328 12.0523C4.01081 11.5711 3.33457 11.3304 3.13309 10.9655C2.95849 10.6492 2.95032 10.2673 3.11124 9.94388C3.29694 9.57063 3.96228 9.30132 5.29295 8.76272L17.8356 3.68594C19.1461 3.15547 19.8014 2.89024 20.2154 3.02623C20.5747 3.14427 20.8565 3.42608 20.9746 3.7854C21.1106 4.19937 20.8453 4.85465 20.3149 6.16521L15.2381 18.7078C14.6995 20.0385 14.4302 20.7039 14.0569 20.8896C13.7335 21.0505 13.3516 21.0423 13.0353 20.8677C12.6704 20.6662 12.4297 19.99 11.9485 18.6375L10.4751 14.4967C10.3815 14.2336 10.3347 14.102 10.2582 13.9922C10.1905 13.8948 10.106 13.8103 10.0086 13.7426C9.89876 13.6661 9.76719 13.6193 9.50407 13.5257L5.36328 12.0523Z"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                    </g>
                </svg>
            );
        case "file-missing":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            d="M19.3517 7.61665L15.3929 4.05375C14.2651 3.03868 13.7012 2.53114 13.0092 2.26562L13 5.00011C13 7.35713 13 8.53564 13.7322 9.26787C14.4645 10.0001 15.643 10.0001 18 10.0001H21.5801C21.2175 9.29588 20.5684 8.71164 19.3517 7.61665Z"
                            fill="#1C274C"
                            className={className}
                        ></path>
                        <path
                            d="M14 22H10C6.22876 22 4.34315 22 3.17157 20.8284C2.72548 20.3823 2.44924 19.8327 2.27818 19.1149C2.15904 18.6149 2.09948 18.3649 2.21429 18.0728C2.32911 17.7806 2.58217 17.6119 3.08829 17.2745L3.71429 16.8571C4.49285 16.3381 5.50715 16.3381 6.28571 16.8571C7.3238 17.5492 8.6762 17.5492 9.71429 16.8571C10.4929 16.3381 11.5071 16.3381 12.2857 16.8571C13.3238 17.5492 14.6762 17.5492 15.7143 16.8571C16.4929 16.3381 17.5071 16.3381 18.2857 16.8571C19.3238 17.5492 20.6762 17.5492 21.7143 16.8571C21.8241 16.7839 21.9705 16.8681 21.965 17C21.8873 18.8723 21.6366 20.0203 20.8284 20.8284C19.6569 22 17.7712 22 14 22Z"
                            fill="#1C274C"
                            className={className}
                        ></path>
                        <path
                            d="M2 10C2 6.22876 2 4.34315 3.17157 3.17157C4.34315 2 6.23869 2 10.0298 2C10.6358 2 11.1214 2 11.53 2.01666C11.5166 2.09659 11.5095 2.17813 11.5092 2.26057L11.5 5.09497C11.4999 6.19207 11.4998 7.16164 11.6049 7.94316C11.7188 8.79028 11.9803 9.63726 12.6716 10.3285C13.3628 11.0198 14.2098 11.2813 15.0569 11.3952C15.8385 11.5003 16.808 11.5002 17.9051 11.5001L18 11.5001H21.9934V12.3273C21.9973 12.5399 21.8912 12.7392 21.7143 12.8571C20.6762 13.5492 19.3238 13.5492 18.2857 12.8571C17.5071 12.3381 16.4929 12.3381 15.7143 12.8571C14.6762 13.5492 13.3238 13.5492 12.2857 12.8571C11.5071 12.3381 10.4929 12.3381 9.71429 12.8571C8.6762 13.5492 7.3238 13.5492 6.28571 12.8571C5.50715 12.3381 4.49285 12.3381 3.71429 12.8571L3.5547 12.9635C2.87033 13.4198 2.52814 13.6479 2.26407 13.5066C2 13.3653 2 12.954 2 12.1315V10Z"
                            fill="#1C274C"
                            className={className}
                        ></path>
                    </g>
                </svg>
            );
        case "warning":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            d="M12 7V13M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12Z"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                        <circle
                            className={className}
                            cx="12"
                            cy="16.5"
                            r="1"
                            fill="#000000"
                        ></circle>
                    </g>
                </svg>
            );
        case "error":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 512 512"
                    version="1.1"
                    fill="#000000"
                >
                    <g>
                        <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                            <g fill="#000000" transform="translate(42.666667, 42.666667)">
                                <path
                                    d="M213.333333,3.55271368e-14 C331.136,3.55271368e-14 426.666667,95.5306667 426.666667,213.333333 C426.666667,331.136 331.136,426.666667 213.333333,426.666667 C95.5306667,426.666667 3.55271368e-14,331.136 3.55271368e-14,213.333333 C3.55271368e-14,95.5306667 95.5306667,3.55271368e-14 213.333333,3.55271368e-14 Z M213.333333,42.6666667 C119.232,42.6666667 42.6666667,119.232 42.6666667,213.333333 C42.6666667,307.434667 119.232,384 213.333333,384 C307.434667,384 384,307.434667 384,213.333333 C384,119.232 307.434667,42.6666667 213.333333,42.6666667 Z M262.250667,134.250667 L292.416,164.416 L243.498667,213.333333 L292.416,262.250667 L262.250667,292.416 L213.333333,243.498667 L164.416,292.416 L134.250667,262.250667 L183.168,213.333333 L134.250667,164.416 L164.416,134.250667 L213.333333,183.168 L262.250667,134.250667 Z"
                                    className={className}
                                ></path>
                            </g>
                        </g>
                    </g>
                </svg>
            );
        case "image-edit":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            d="M14.2639 15.9376L12.5958 14.2835C11.7909 13.4852 11.3884 13.0861 10.9266 12.9402C10.5204 12.8119 10.0838 12.8166 9.68048 12.9537C9.22188 13.1096 8.82814 13.5173 8.04068 14.3327L4.04409 18.2802M14.2639 15.9376L14.6053 15.5991C15.4112 14.7999 15.8141 14.4003 16.2765 14.2544C16.6831 14.1262 17.12 14.1312 17.5236 14.2688C17.9824 14.4252 18.3761 14.834 19.1634 15.6515L20 16.4936M14.2639 15.9376L18.275 19.9566M18.275 19.9566C17.9176 20.0001 17.4543 20.0001 16.8 20.0001H7.2C6.07989 20.0001 5.51984 20.0001 5.09202 19.7821C4.71569 19.5904 4.40973 19.2844 4.21799 18.9081C4.12796 18.7314 4.07512 18.5322 4.04409 18.2802M18.275 19.9566C18.5293 19.9257 18.7301 19.8728 18.908 19.7821C19.2843 19.5904 19.5903 19.2844 19.782 18.9081C20 18.4803 20 17.9202 20 16.8001V16.4936M12.5 4L7.2 4.00011C6.07989 4.00011 5.51984 4.00011 5.09202 4.21809C4.71569 4.40984 4.40973 4.7158 4.21799 5.09213C4 5.51995 4 6.08 4 7.20011V16.8001C4 17.4576 4 17.9222 4.04409 18.2802M20 11.5V16.4936M14 10.0002L16.0249 9.59516C16.2015 9.55984 16.2898 9.54219 16.3721 9.5099C16.4452 9.48124 16.5146 9.44407 16.579 9.39917C16.6515 9.34859 16.7152 9.28492 16.8425 9.1576L21 5.00015C21.5522 4.44787 21.5522 3.55244 21 3.00015C20.4477 2.44787 19.5522 2.44787 19 3.00015L14.8425 7.1576C14.7152 7.28492 14.6515 7.34859 14.6009 7.42112C14.556 7.4855 14.5189 7.55494 14.4902 7.62801C14.4579 7.71033 14.4403 7.79862 14.4049 7.97518L14 10.0002Z"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            className={className}
                        ></path>
                    </g>
                </svg>
            );
        case "edit":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g>
                        <g>
                            <g>
                                <g>
                                    <path
                                        className={className}
                                        d="M20,16v4a2,2,0,0,1-2,2H4a2,2,0,0,1-2-2V6A2,2,0,0,1,4,4H8"
                                        fill="none"
                                        stroke="#000000"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        strokeWidth="2"
                                    ></path>
                                    <polygon
                                        className={className}
                                        fill="none"
                                        points="12.5 15.8 22 6.2 17.8 2 8.3 11.5 8 16 12.5 15.8"
                                        stroke="#000000"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                        strokeWidth="2"
                                    ></polygon>
                                </g>
                            </g>
                        </g>
                    </g>
                </svg>
            );
        case "sold":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    id="Layer_1"
                    data-name="Layer 1"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g>
                        <path
                            className={className}
                            d="M1.46,19.19H4a1.25,1.25,0,0,0,1.25-1.25h0a1.26,1.26,0,0,0-.55-1L3,15.72a1.25,1.25,0,0,1-.55-1h0a1.25,1.25,0,0,1,1.25-1.25H6.25"
                        ></path>
                        <line className={className} x1="23.5" y1="22.06" x2="0.5" y2="22.06"></line>
                        <polyline
                            className={className}
                            points="22.54 10.56 12 1.94 1.46 10.56"
                        ></polyline>
                        <rect
                            className={className}
                            x="8.17"
                            y="13.44"
                            width="2.88"
                            height="5.75"
                            rx="1.44"
                        ></rect>
                        <polyline
                            className={className}
                            points="13.92 12.48 13.92 19.19 16.79 19.19"
                        ></polyline>
                        <path
                            className={className}
                            d="M18.71,13.44h1a1.92,1.92,0,0,1,1.92,1.92v1.92a1.92,1.92,0,0,1-1.92,1.92h-1a0,0,0,0,1,0,0V13.44a0,0,0,0,1,0,0Z"
                        ></path>
                    </g>
                </svg>
            );
        case "trash":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            d="M10 12L14 16M14 12L10 16M4 6H20M16 6L15.7294 5.18807C15.4671 4.40125 15.3359 4.00784 15.0927 3.71698C14.8779 3.46013 14.6021 3.26132 14.2905 3.13878C13.9376 3 13.523 3 12.6936 3H11.3064C10.477 3 10.0624 3 9.70951 3.13878C9.39792 3.26132 9.12208 3.46013 8.90729 3.71698C8.66405 4.00784 8.53292 4.40125 8.27064 5.18807L8 6M18 6V16.2C18 17.8802 18 18.7202 17.673 19.362C17.3854 19.9265 16.9265 20.3854 16.362 20.673C15.7202 21 14.8802 21 13.2 21H10.8C9.11984 21 8.27976 21 7.63803 20.673C7.07354 20.3854 6.6146 19.9265 6.32698 19.362C6 18.7202 6 17.8802 6 16.2V6"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                    </g>
                </svg>
            );
        case "send":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            d="M10.3009 13.6949L20.102 3.89742M10.5795 14.1355L12.8019 18.5804C13.339 19.6545 13.6075 20.1916 13.9458 20.3356C14.2394 20.4606 14.575 20.4379 14.8492 20.2747C15.1651 20.0866 15.3591 19.5183 15.7472 18.3818L19.9463 6.08434C20.2845 5.09409 20.4535 4.59896 20.3378 4.27142C20.2371 3.98648 20.013 3.76234 19.7281 3.66167C19.4005 3.54595 18.9054 3.71502 17.9151 4.05315L5.61763 8.2523C4.48114 8.64037 3.91289 8.83441 3.72478 9.15032C3.56153 9.42447 3.53891 9.76007 3.66389 10.0536C3.80791 10.3919 4.34498 10.6605 5.41912 11.1975L9.86397 13.42C10.041 13.5085 10.1295 13.5527 10.2061 13.6118C10.2742 13.6643 10.3352 13.7253 10.3876 13.7933C10.4468 13.87 10.491 13.9585 10.5795 14.1355Z"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                    </g>
                </svg>
            );
        case "three-dots":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 16 16"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g>
                        <path
                            className={className}
                            d="M9.5 13a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0z"
                        ></path>
                    </g>
                </svg>
            );
        case "help":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12ZM12 17.75C12.4142 17.75 12.75 17.4142 12.75 17V11C12.75 10.5858 12.4142 10.25 12 10.25C11.5858 10.25 11.25 10.5858 11.25 11V17C11.25 17.4142 11.5858 17.75 12 17.75ZM12 7C12.5523 7 13 7.44772 13 8C13 8.55228 12.5523 9 12 9C11.4477 9 11 8.55228 11 8C11 7.44772 11.4477 7 12 7Z"
                            fill="#1C274C"
                        ></path>
                    </g>
                </svg>
            );
        case "star":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            className={className}
                            d="M11.2691 4.41115C11.5006 3.89177 11.6164 3.63208 11.7776 3.55211C11.9176 3.48263 12.082 3.48263 12.222 3.55211C12.3832 3.63208 12.499 3.89177 12.7305 4.41115L14.5745 8.54808C14.643 8.70162 14.6772 8.77839 14.7302 8.83718C14.777 8.8892 14.8343 8.93081 14.8982 8.95929C14.9705 8.99149 15.0541 9.00031 15.2213 9.01795L19.7256 9.49336C20.2911 9.55304 20.5738 9.58288 20.6997 9.71147C20.809 9.82316 20.8598 9.97956 20.837 10.1342C20.8108 10.3122 20.5996 10.5025 20.1772 10.8832L16.8125 13.9154C16.6877 14.0279 16.6252 14.0842 16.5857 14.1527C16.5507 14.2134 16.5288 14.2807 16.5215 14.3503C16.5132 14.429 16.5306 14.5112 16.5655 14.6757L17.5053 19.1064C17.6233 19.6627 17.6823 19.9408 17.5989 20.1002C17.5264 20.2388 17.3934 20.3354 17.2393 20.3615C17.0619 20.3915 16.8156 20.2495 16.323 19.9654L12.3995 17.7024C12.2539 17.6184 12.1811 17.5765 12.1037 17.56C12.0352 17.5455 11.9644 17.5455 11.8959 17.56C11.8185 17.5765 11.7457 17.6184 11.6001 17.7024L7.67662 19.9654C7.18404 20.2495 6.93775 20.3915 6.76034 20.3615C6.60623 20.3354 6.47319 20.2388 6.40075 20.1002C6.31736 19.9408 6.37635 19.6627 6.49434 19.1064L7.4341 14.6757C7.46898 14.5112 7.48642 14.429 7.47814 14.3503C7.47081 14.2807 7.44894 14.2134 7.41394 14.1527C7.37439 14.0842 7.31195 14.0279 7.18708 13.9154L3.82246 10.8832C3.40005 10.5025 3.18884 10.3122 3.16258 10.1342C3.13978 9.97956 3.19059 9.82316 3.29993 9.71147C3.42581 9.58288 3.70856 9.55304 4.27406 9.49336L8.77835 9.01795C8.94553 9.00031 9.02911 8.99149 9.10139 8.95929C9.16534 8.93081 9.2226 8.8892 9.26946 8.83718C9.32241 8.77839 9.35663 8.70162 9.42508 8.54808L11.2691 4.41115Z"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                    </g>
                </svg>
            );
        case "share":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <g>
                            <path
                                className={className}
                                d="M9 6L12 3M12 3L15 6M12 3V13M7.00023 10C6.06835 10 5.60241 10 5.23486 10.1522C4.74481 10.3552 4.35523 10.7448 4.15224 11.2349C4 11.6024 4 12.0681 4 13V17.8C4 18.9201 4 19.4798 4.21799 19.9076C4.40973 20.2839 4.71547 20.5905 5.0918 20.7822C5.5192 21 6.07899 21 7.19691 21H16.8036C17.9215 21 18.4805 21 18.9079 20.7822C19.2842 20.5905 19.5905 20.2839 19.7822 19.9076C20 19.4802 20 18.921 20 17.8031V13C20 12.0681 19.9999 11.6024 19.8477 11.2349C19.6447 10.7448 19.2554 10.3552 18.7654 10.1522C18.3978 10 17.9319 10 17 10"
                                stroke="#000000"
                                strokeWidth="2"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                            ></path>
                        </g>
                    </g>
                </svg>
            );
        case "information":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <g clip-path="url(#clip0_429_11160)">
                            <circle
                                className={className}
                                cx="12"
                                cy="11.9999"
                                r="9"
                                stroke="#292929"
                                strokeWidth="1.6799999999999997"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                            ></circle>
                            <rect
                                className={className}
                                x="12"
                                y="8"
                                width="0.01"
                                height="0.01"
                                stroke="#292929"
                                strokeWidth="1.6799999999999997"
                                strokeLinejoin="round"
                            ></rect>
                            <path
                                className={className}
                                d="M12 12V16"
                                stroke="#292929"
                                strokeWidth="1.6799999999999997"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                            ></path>
                        </g>
                        <defs>
                            <clipPath>
                                <rect
                                    className={className}
                                    width="24"
                                    height="24"
                                    fill="white"
                                ></rect>
                            </clipPath>
                        </defs>
                    </g>
                </svg>
            );
        case "facebook":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 48 48"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g>
                        <g
                            id="Icons"
                            stroke="none"
                            stroke-width="1"
                            fill="none"
                            fill-rule="evenodd"
                        >
                            <g
                                id="Color-"
                                transform="translate(-200.000000, -160.000000)"
                                fill="#4460A0"
                            >
                                <path
                                    d="M225.638355,208 L202.649232,208 C201.185673,208 200,206.813592 200,205.350603 L200,162.649211 C200,161.18585 201.185859,160 202.649232,160 L245.350955,160 C246.813955,160 248,161.18585 248,162.649211 L248,205.350603 C248,206.813778 246.813769,208 245.350955,208 L233.119305,208 L233.119305,189.411755 L239.358521,189.411755 L240.292755,182.167586 L233.119305,182.167586 L233.119305,177.542641 C233.119305,175.445287 233.701712,174.01601 236.70929,174.01601 L240.545311,174.014333 L240.545311,167.535091 C239.881886,167.446808 237.604784,167.24957 234.955552,167.24957 C229.424834,167.24957 225.638355,170.625526 225.638355,176.825209 L225.638355,182.167586 L219.383122,182.167586 L219.383122,189.411755 L225.638355,189.411755 L225.638355,208 L225.638355,208 Z"
                                    id="Facebook"
                                ></path>
                            </g>
                        </g>
                    </g>
                </svg>
            );
        case "twitter":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 -4 48 48"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#000000"
                >
                    <g>
                        <g
                            id="Icons"
                            stroke="none"
                            stroke-width="1"
                            fill="none"
                            fill-rule="evenodd"
                        >
                            <g
                                id="Color-"
                                transform="translate(-300.000000, -164.000000)"
                                fill="#00AAEC"
                            >
                                <path
                                    d="M348,168.735283 C346.236309,169.538462 344.337383,170.081618 342.345483,170.324305 C344.379644,169.076201 345.940482,167.097147 346.675823,164.739617 C344.771263,165.895269 342.666667,166.736006 340.418384,167.18671 C338.626519,165.224991 336.065504,164 333.231203,164 C327.796443,164 323.387216,168.521488 323.387216,174.097508 C323.387216,174.88913 323.471738,175.657638 323.640782,176.397255 C315.456242,175.975442 308.201444,171.959552 303.341433,165.843265 C302.493397,167.339834 302.008804,169.076201 302.008804,170.925244 C302.008804,174.426869 303.747139,177.518238 306.389857,179.329722 C304.778306,179.280607 303.256911,178.821235 301.9271,178.070061 L301.9271,178.194294 C301.9271,183.08848 305.322064,187.17082 309.8299,188.095341 C309.004402,188.33225 308.133826,188.450704 307.235077,188.450704 C306.601162,188.450704 305.981335,188.390033 305.381229,188.271578 C306.634971,192.28169 310.269414,195.2026 314.580032,195.280607 C311.210424,197.99061 306.961789,199.605634 302.349709,199.605634 C301.555203,199.605634 300.769149,199.559408 300,199.466956 C304.358514,202.327194 309.53689,204 315.095615,204 C333.211481,204 343.114633,188.615385 343.114633,175.270495 C343.114633,174.831347 343.106181,174.392199 343.089276,173.961719 C345.013559,172.537378 346.684275,170.760563 348,168.735283"
                                    id="Twitter"
                                ></path>
                            </g>
                        </g>
                    </g>
                </svg>
            );
        case "file-upload":
            return (
                <svg
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <g>
                        <path
                            d="M12 18V12M12 12L14 14M12 12L10 14M13 3H8.2C7.0799 3 6.51984 3 6.09202 3.21799C5.71569 3.40973 5.40973 3.71569 5.21799 4.09202C5 4.51984 5 5.0799 5 6.2V17.8C5 18.9201 5 19.4802 5.21799 19.908C5.40973 20.2843 5.71569 20.5903 6.09202 20.782C6.51984 21 7.0799 21 8.2 21H15.8C16.9201 21 17.4802 21 17.908 20.782C18.2843 20.5903 18.5903 20.2843 18.782 19.908C19 19.4802 19 18.9201 19 17.8V9M13 3L19 9M13 3V7.4C13 7.96005 13 8.24008 13.109 8.45399C13.2049 8.64215 13.3578 8.79513 13.546 8.89101C13.7599 9 14.0399 9 14.6 9H19"
                            stroke="#000000"
                            strokeWidth="2"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ></path>
                    </g>
                </svg>
            );
        case "heating":
            return (
                <svg
                    fill="#000000"
                    width={width || DEFAULT_W}
                    height={height || DEFAULT_H}
                    className={className}
                    version="1.1"
                    id="Layer_1"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                >
                    <g id="SVGRepo_iconCarrier">
                        <g>
                            <g>
                                <path d="M465.083,99.294c-5.161,0-10.125,0.85-14.774,2.397c-1.146-36.95-31.539-66.66-68.762-66.66 c-20.817,0-39.492,9.305-52.122,23.959c-12.63-14.654-31.304-23.959-52.122-23.959c-20.818,0-39.492,9.305-52.122,23.958 c-12.63-14.654-31.303-23.958-52.122-23.958s-39.492,9.305-52.122,23.958c-12.63-14.653-31.304-23.958-52.122-23.958 C30.871,35.031,0,65.902,0,103.848v304.304c0,37.946,30.871,68.817,68.817,68.817c20.817,0,39.492-9.305,52.122-23.958 c12.63,14.654,31.304,23.958,52.122,23.958c20.817,0,39.492-9.305,52.122-23.958c12.63,14.654,31.304,23.958,52.122,23.958 s39.492-9.305,52.122-23.959c12.63,14.654,31.304,23.959,52.122,23.959c37.946,0,68.816-30.871,68.816-68.817V190.749 c4.632,1.535,9.578,2.379,14.72,2.379c25.87,0,46.917-21.047,46.917-46.916C512,120.34,490.953,99.294,465.083,99.294z M104.243,408.152c0,19.534-15.892,35.426-35.426,35.426s-35.426-15.892-35.426-35.426V103.848 c0-19.534,15.892-35.426,35.426-35.426s35.426,15.892,35.426,35.426V408.152z M208.486,408.152 c0,19.534-15.892,35.426-35.426,35.426c-19.534,0-35.426-15.892-35.426-35.426V103.848c0-19.534,15.892-35.426,35.426-35.426 c19.534,0,35.426,15.892,35.426,35.426V408.152z M312.73,408.152c0,19.534-15.893,35.426-35.426,35.426 c-19.534,0-35.426-15.892-35.426-35.426V103.848c0-19.534,15.892-35.426,35.426-35.426s35.426,15.892,35.426,35.426V408.152z M416.972,408.152c0,19.534-15.891,35.426-35.425,35.426c-19.534,0-35.426-15.892-35.426-35.426V103.848 c0-19.534,15.892-35.426,35.426-35.426s35.425,15.892,35.425,35.426V408.152z M465.083,159.736 c-7.457,0-13.525-6.067-13.525-13.525c0-7.458,6.067-13.526,13.525-13.526c7.458,0,13.526,6.067,13.526,13.526 C478.609,153.669,472.542,159.736,465.083,159.736z"></path>{" "}
                            </g>
                        </g>
                    </g>
                </svg>
            );
    }
}
